...
3. This continuous representation is sent through an Add & Norm layer. Addition layer is placed to solve the vanishing gradient problem. This is by residual addition from the input vector. The residual connection shortens the effective gradient path because it bypasses layers — so it's one mechanism solving one core problem.
...
```
The Encoder stack isn't just one encoder — it's N encoders stacked (typically 6 in the original "Attention Is All You Need" paper). The output of one feeds into the next, progressively refining the representation.
```
